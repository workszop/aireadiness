...
- Polish and English (`questions.txt`, `questions_en.txt`). One line per question: `question|best option|...|worst option`, sections start with `#`.
...
- Single `index.html`, no build step. Progress is kept in localStorage. Serve with `python3 -m http.server` for local testing (the question files are fetched).
